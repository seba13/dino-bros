...
var cercas = []; // recorrer array de posiciones (coordenada x) de cada cerca
...
propGenerales.cerca.posicionX.forEach(function (cerca) {
  for (var _index2 = 0; _index2 < cerca.cantidad; _index2++) {
    cercas.push(new Sprite({
      posicion: {
        x: cerca.x + propGenerales.cerca.ancho * _index2,
...
function iniciar() {
  window.addEventListener("resize", function () {
    canvas.width = window.innerWidth;
    canvas.height = window.innerHeight;
    var proporcion = 1920 / canvas.width; // canvas.style.transform = `scale${proporcion}`
    // fondo 1008 => 100
    // 1920  => x
    // nubes
    // cesped
  ...
  function animar() {
    ...
    for (var _index3 in nubesPequeñas) {
      nubesPequeñas[_index3].actualizarSprite();

      nubesGrandes[_index3].actualizarSprite();
    ...
    suelos.forEach(function (suelo) {
      return suelo.actualizarSprite();
    }); // suelo.actualizarSprite();

    cespeds.forEach(function (s) {
      return s.actualizarSprite();
    });
    flores.forEach(function (flor) {
      return flor.actualizarSprite();
    });
    cercas.forEach(function (cerca) {
      return cerca.actualizarSprite();
    });
